...
public void paint(Graphics gc){
	    ...
	    //double w  = getParent().getBounds().getWidth();
	    double h  = getParent().getBounds().getHeight();
	    double w = image.getWidth() / (image.getHeight() / h);
	    setPreferredSize(new Dimension((int)w,(int)h));

	    double sx = w/image.getWidth();
	    double sy = h/image.getHeight();
	    g.drawRenderedImage(image,java.awt.geom.AffineTransform.getScaleInstance(sx,sy)); 
  }
  ...
  public void rotate(){
    ...
    try{
      ...
      AffineTransform rotate = new AffineTransform(0.0,1.0,-1.0,0.0,oldimage.getHeight(),0.0);
